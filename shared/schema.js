...
export const wallapopChats = pgTable("wallapop_chats", {
  ...
  productId: varchar("product_id")
    .references(() => wallapopProducts.id)
    .notNull(),
...
export const darkwebUsers = pgTable("darkweb_users", {
  ...
  handle: text("handle")
    .notNull()
    .unique(), // Anonymous handle like "AnonymousUser123"
...
// Insert schemas
export const insertInstagramUserSchema = createInsertSchema(
  instagramUsers
).omit({ id: true, createdAt: true })
export const insertInstagramPostSchema = createInsertSchema(
  instagramPosts
).omit({ id: true, likes: true, commentsCount: true, createdAt: true })
export const insertInstagramCommentSchema = createInsertSchema(
  instagramComments
).omit({ id: true, createdAt: true })
export const insertInstagramFollowSchema = createInsertSchema(
  instagramFollows
).omit({ id: true, createdAt: true })
export const insertInstagramStorySchema = createInsertSchema(
  instagramStories
).omit({ id: true, createdAt: true })
export const insertTinderUserSchema = createInsertSchema(tinderUsers).omit({
  id: true,
  createdAt: true
})
export const insertTinderMatchSchema = createInsertSchema(tinderMatches).omit({
  id: true,
  swipedAt: true
})
export const insertWallapopUserSchema = createInsertSchema(wallapopUsers).omit({
  id: true,
  rating: true,
  createdAt: true
})
export const insertWallapopProductSchema = createInsertSchema(
  wallapopProducts
).omit({ id: true, views: true, sold: true, createdAt: true })
export const insertWallapopChatSchema = createInsertSchema(wallapopChats).omit({
  id: true,
  lastMessage: true,
  lastMessageAt: true,
  createdAt: true
})
export const insertWallapopMessageSchema = createInsertSchema(
  wallapopMessages
...
export const insertDarkwebUserSchema = createInsertSchema(darkwebUsers).omit({
  id: true,
  reputation: true,
  isOnline: true,
  lastSeen: true,
...
export const insertDarkwebChannelSchema = createInsertSchema(
  darkwebChannels
).omit({ id: true, createdAt: true })
export const insertDarkwebMessageSchema = createInsertSchema(
  darkwebMessages
).omit({ id: true, sentAt: true })
...
export const insertRegistrationCodeSchema = createInsertSchema(
  registrationCodes
