...
export default function Tinder({ onBack }) {
  ...
  const swipeMutation = useMutation({
    mutationFn: async ({ userId, targetUserId, isLike }) => {
  ...
  const handleRegister = (e) => {
    ...
    registerMutation.mutate({
      ...
      interests: formData.get("interests") ? formData.get("interests").split(',').map(i => i.trim()).filter(Boolean) : [],
  ...
  const handleSwipe = (targetUserId, isLike) => {
    if (currentUser) {
